import { InputType, KeyEventF, VKC, KeysAsSimbols } from '../utils/enums';
import { KeyBoardStruct } from '../utils/structs';
import { kUser32, _CBSIZE } from './FFI';

function SendKeyEvent(key: VKC, evtType: KeyEventF, time: number = 0, debug: boolean = false): void {
...
export function SendUpperKey(key: VKC, time: number = 0, debug: boolean = false) {
...
export function SendKeyChain(keyChain: string): void {
    ...
    let newKeyArray:any[] = keyArray.map(char => 
        (char in KeysAsSimbols) ? char = KeysAsSimbols[char] : char = char &&
        (!isNaN(char)) ? char = `K${char}` : char = char
    )
